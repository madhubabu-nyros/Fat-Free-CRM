...
class LeadsController < EntitiesController
  ...
  def index
    ...
    # @leads = get_leads.paginate(:page => params[:page], :per_page => 10)  

    respond_with @leads do |format|
      format.xls { render layout: 'header' }
      format.csv { render csv: @leads }
      format.pdf do
        pdf = LeadPdf.new(@leads)
        send_data pdf.render, filename: 'leads.pdf', type: 'application/pdf', disposition: 'inline'
      end
    end
  end
